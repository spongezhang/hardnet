...
def hard_mining(test_loader, epoch, logger, logger_test_name):
    
    num_sample = test_loader.dataset.data.shape[0]
    descriptor = test_loader.dataset.descriptor
    descriptor = descriptor.astype(float)
    descriptor_p = np.power(descriptor,2)
    descriptor /= np.sqrt(descriptor_p.sum(axis=1, keepdims=True) + 1e-7)
    ...
    bad_num = 0
    bad_distance = 0
    all_label = test_loader.dataset.labels.numpy()
    for i in range(num_sample):
        retrieved_label = all_label[I[i,:]]
        bad_num += np.sum(np.not_equal(retrieved_label,all_label[i]))
        bad_distance += np.sum(D[i,np.not_equal(retrieved_label,all_label[i])])
    print(bad_num)
...
def main(test_loaders, logger, file_logger):
    ...
    for test_loader in test_loaders:
        ...
        hard_mining(test_loader['dataloader'], 0, logger, test_loader['name'])
