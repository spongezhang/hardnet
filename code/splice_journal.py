...
class splice_journal(data.Dataset):
    ...
    def __init__(self, root, name, train=True, transform=None, download=False):
        ...
        self.data_file = os.path.join(self.root, '{}_splice.pt'.format(name))
...
def read_image_file(data_dir, dataset_name):
    ...
    patches = hickle.load(os.path.join(data_dir, dataset_name+'_splice_patch.dat'))
    #print(patches.shape)
...
def read_info_file(data_dir, dataset_name):
    ...
    """
    labels = hickle.load(os.path.join(data_dir, dataset_name+'_splice_label.dat'))
