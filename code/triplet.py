...
#from Losses import loss_margin_min, loss_margin_min_t, loss_margin_min_gor
import Losses
...
parser.add_argument('--alpha', type=float, default=1.0 , metavar='BS',
                    help='input batch size for training (default: 128)')
parser.add_argument('--test-batch-size', type=int, default=2000, metavar='BST',
...
now = datetime.now()
suffix = 'hardnet_LR{:1.0e}_alpha{:1.1e}_margin_{:1.1f}_lossType_{}_'.format(args.lr, args.alpha, args.margin, args.Loss_Type) \
        +  now.strftime("%Y%m%d-%H%M%S")

training_logs_dir = './tensorflow_log/train_{}/'.format(suffix)
test_logs_dir = './tensorflow_log/test_{}/'.format(suffix)

training_writer = tf.summary.FileWriter(training_logs_dir)
test_writer = tf.summary.FileWriter(test_logs_dir)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, (data_a, data_p, data_n) in pbar:
        ...
        if args.Loss_Type == 1:
            loss, gor = Losses.loss_margin_min_gor(out_a, out_p, out_n, alpha = args.alpha, margin=args.margin, anchor_swap=args.anchorswap, anchor_ave=args.anchorave)
        elif args.Loss_Type == 0:
            loss, gor = Losses.loss_margin_min_t(out_a, out_p, out_n,  margin=args.margin, anchor_swap=args.anchorswap, anchor_ave=args.anchorave)
        elif args.Loss_Type == 2:            
            loss, gor = Losses.triplet_margin_loss(out_p, out_a, out_n, margin=args.margin, swap=args.anchorswap)
        else:            
            loss, gor = Losses.triplet_margin_loss_gor(out_p, out_a, out_n, alpha = args.alpha, margin=args.margin, swap=args.anchorswap)
        ...
        if batch_idx % args.log_interval == 0:
            loss_summary = tf.Summary(value=[tf.Summary.Value(tag="loss", simple_value=loss.data[0])])
            training_writer.add_summary(loss_summary, batch_idx+epoch*len(train_loader))
            gor_summary = tf.Summary(value=[tf.Summary.Value(tag="gor", simple_value=gor.data[0])])
            training_writer.add_summary(gor_summary, batch_idx+epoch*len(train_loader))

            #print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, gor: {:.6f}'.format(
                    #epoch, batch_idx * len(data_a), len(train_loader.dataset),
                    #100. * batch_idx / len(train_loader),
                    #loss.data[0], gor.data[0]))
...
def test(test_loader, model, epoch):
    ...
    test_writer.add_summary(fpr_summary, epoch)
